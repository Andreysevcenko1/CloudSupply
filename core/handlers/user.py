...
async def send_order_notification_to_admin(context, order, user, delivery_method, delivery_fee):
    ...
    async with async_session_maker() as session:
        ...
        async with async_session_maker() as session:
            ...
            print(f"[NOTIFICATION] Найдено OrderItem для order_id={order.id}: {len(order_items)} записей")
            for i, item in enumerate(order_items):
                print(f"[NOTIFICATION] OrderItem #{i}: id={item.id}, product_id={item.product_id}, quantity={item.quantity}")
