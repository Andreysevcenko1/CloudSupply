...
async def add_to_cart(query, context):
    ...
    async with async_session_maker() as session:
        ...
        if product.stock_quantity < quantity:
            await query.answer(f"❌ Недостаточно товара! Доступно: {product.stock_quantity} шт", show_alert=True)
            return
        
        await crud.add_to_cart(session, user.id, product_id, quantity)
        model = await crud.get_model_by_id(session, product.model_id)
        
        # Удаляем предыдущее сообщение и отправляем подтверждение
        try:
            await query.message.delete()
        except:
            pass
...
async def show_cart_internal(update, context, user_id, query=None):
    ...
    async with async_session_maker() as session:
        ...
        if not cart_items:
            ...
            text += f"💰 Итого: {total}€"
            keyboard = get_cart_kb(cart_items, has_items=True)
...
async def show_order_detail(query, context):
    ...
    async with async_session_maker() as session:
        ...
        admin = is_admin(query.from_user.username)
        await query.edit_message_text(text, reply_markup=get_order_detail_kb(order_id, is_admin=admin))
...
async def handle_message(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if context.user_data.get('state') and is_admin(update.effective_user.username):
