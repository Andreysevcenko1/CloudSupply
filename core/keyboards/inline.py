...
def get_product_quantity_kb(product_id):
    ...
    keyboard = [
        [
            InlineKeyboardButton("1 шт", callback_data=f"addcart_{product_id}_1"),
            InlineKeyboardButton("2 шт", callback_data=f"addcart_{product_id}_2"),
            InlineKeyboardButton("3 шт", callback_data=f"addcart_{product_id}_3"),
        ],
        [
            InlineKeyboardButton("5 шт", callback_data=f"addcart_{product_id}_5"),
            InlineKeyboardButton("10 шт", callback_data=f"addcart_{product_id}_10"),
        ],
...
def get_cart_kb(cart_items, has_items=True):
    """Клавиатура корзины"""
    keyboard = []
    
    if has_items:
        for item in cart_items:
            text = f"❌ {item.product.flavor_name} ({item.quantity} шт)"
...
def get_order_detail_kb(order_id, is_admin=False):
    """Детали заказа"""
    keyboard = []
